...
Solution::Solution() : oc1(0.9), oc2(0.6), oc3(1.2)
{
}

void Solution::initialize(int _num)
{
	numAvailableUAV = _num;
	totalServed = 0;
	...
	qDebug("margin: %f, countX: %d, countY: %d", margin, countX, countY);
	UAVs.reserve(64);
	...
	qDebug("initialBucketNum: %d, outsideBucketNum: %d\n", initialBucketNum, outsideBucketNum);
}

void Solution::deployOne(std::vector<size_t>& adjustIndices)
{
	qDebug("=========================    deploy UAV %lu    =========================\n", UAVs.size()+1);
	...
	for (size_t idx = 0; idx < UAVs.size()-1; ++idx)
	{
		...
		if (distO2O < distRpR)
		{
			__adjustUAV(idx, adjustIndices);
			qDebug("UAV num: %lu, total served num: %d\n", UAVs.size(), totalServed);
			break;
		}
	}
...
int Solution::__findMaxCanServePlace(double &maxX, double &maxY, double R)
{
	...
	// qDebug("maxUnservedNearby: %d, position: (%.2f, %.2f)\n", maxUnservedNearby, maxX, maxY);
	return maxUnservedNearby;
}

void Solution::__adjustUAV(size_t uavIdx, std::vector<size_t>& adjustIndices)
{
	UAV &curUAV = UAVs.back();
	...
	qDebug("conflicted UAV num: %lu, conflicted band num: %lu\n", conflictedUAVs.size(), conflictedBandNum);
	...
	{
		qDebug("overlap is less than 20%%, maxOverlap: %f\n", maxOverlap);
		...
		if (conflictedBandNum == UAV::bandNum)
		{
			if (UAV::bandNum > 1)
			{
				...
				qDebug("conflictedBandNum == UAV::bandNum && UAV::bandNum > 1, choose band %d to avoid.\n", bandToAvoid);
			}
		}
...
void Solution::__alterUAV(size_t uavIdx)
{
	...
	{
		qDebug("adjust both oldUAV and curUAV.\n");
		...
		qDebug("newR: %f, maxUnservedNum: %lu, optimalAngle: %f\n", newR, maxUnservedNum, optimalAngle);
		...
		qDebug("move [old]: (%.2f,%.2f), R: %.2f, N: %d  ==>  (%.2f,%.2f), R: %.2f, N: %d", oldPos.x, oldPos.y, oldR, oldServedNum, oldUAV.getX(), oldUAV.getY(), oldUAV.getR(), oldUAV.userNum());
		qDebug("move [cur]: (%.2f,%.2f), R: %.2f, N: %d  ==>  (%.2f,%.2f), R: %.2f, N: %d\n", curPos.x, curPos.y, curR, curServedNum, curUAV.getX(), curUAV.getY(), curUAV.getR(), curUAV.userNum());
	...
	else // only adjust curUAV
	{
		...
		{
			qDebug("maxUnservedNum: %d, optimalAngle: %f\n", maxUnservedNum, optimalAngle);
		...
		else
		{
			newPos = curPos;
			if (optimalAngle < -1.0)
				qDebug("no valid angle can be found, undo rotation\n");
			else
				qDebug("maxUnservedNum: %d is too small, undo rotation\n", maxUnservedNum);
		}
...
bool Solution::__tangentUAV(size_t uavIdx, Point& newPos, std::vector<size_t> conflictedUAVs)
{
	...
	if (conflictedUAVs.size() == 3)
	{
		...
		if (!math::equal0(kAB - kAC) && math::dist(A, B) >= rA + rB && math::dist(B, C) >= rB + rC && math::dist(C, A) >= rC + rA)
		{
			...
			else
			{
				if (newR < minRadius)
				{
					qDebug("new radius is too small.\n");
					return tangentExecuted;
				}
				else if (newR > maxRadius)
					qDebug("new radius is too large.\n");
				else
					qDebug("moving distance is too large.\n");
			}
		}
		else if (math::equal0(kAB - kAC))
			qDebug("point A, B, C on one line.\n");
		else
			qDebug("there exists intersection between circles.\n");
		if (!tangentExecuted)
		{
	...
	if (!tangentExecuted && conflictedUAVs.size() == 2)
	{
		...
		if (maxReduceR > 0)
		{
			qDebug("three tangent circles [again].");
...
void Solution::__handleBuckets(int user, bool serve)
{
	...
	if (serve)
	{
		...
		Q_ASSERT(itUS != buckets[_xIdx][_yIdx].end());

...
int UAV::serve(int user)
{
	...
	B += bwReq;
	servedTable[user] = 1;
	// qDebug("serve user [%d], now B is %f\n", user, B);
	return Status::OK;
}
...
int UAV::unserve(int user)
{
	if ((itU = users.find(user)) != users.end())
	{
		B -= itU->second;
		servedTable[user] = 0;
		// qDebug("unserve user [%d], now B is %f\n", user, B);
		users.erase(itU);
		return Status::OK;
	}
...
void UAV::getUserList(std::list<int>& userList)
{
	userList.clear();
	for (itU = users.begin(); itU != users.end(); ++itU)
		userList.push_back(itU->first);
}
...
void UAV::setBand(int _band)
{
	...
	case Band::_4_4:
		...
		break;
	default:
		qCritical("Unknown operating band!\n");
		exit(EXIT_FAILURE);
	}
	if (band != -1 && type != oldType)
	{
		qCritical("cannot change to different band type.\n");
		exit(EXIT_FAILURE);
	}
	band = _band;
}
...
void configureUAV()
{
	...
	for (int type = 0; type < 2; ++type)
	{
		...
		qDebug("total bandwidth %fkHz  <==>  total capacity: %fkbps", UAV::totalB[type], UAV::totalC[type]);
	}
}

...
Point circleCircle(const Point& A, const Point& B, const Point &C, double rA, double rB, double rC)
{
	...
	qDebug("cosA: %f, sinA: %f, AD: %f, CD: %f", cosA, sinA, AD, CD);
...
Point pointPointCircle(const Point& A, const Point& B, const Point& I, double rI)
{
	...
	qDebug("A: (%f,%f), B: (%f,%f), D: (%f,%f)", A.x, A.y, B.x, B.y, D.x, D.y);
	if (equal0(A.y - B.y))
	{
		if (equal0(D.x - I.x))
		{
	...
	qDebug("I: (%f,%f), P:(%f,%f), rI: %f, rP: %f, C: (%f,%f)", I.x, I.y, P.x, P.y, rI, rP, C.x, C.y);
	...
	qDebug("k1: %f, k2: %f, T1: (%f,%f), T2:(%f,%f)", k1, k2, T1.x, T1.y, T2.x, T2.y);
...
Point pointCircleCircle(const Point& A, const Point& B, const Point& C, double rA, double rB)
{
	...
	qDebug("S1: (%f,%f), S2:(%f,%f), P: (%f,%f), Q:(%f,%f)", S1.x, S1.y, S2.x, S2.y, P.x, P.y, Q.x, Q.y);
